properties([
    ...
    [$class: 'GithubProjectProperty', displayName: '', projectUrlStr: 'https://github.com/ossimlabs/omar-download'],
...
)
{
  node(POD_LABEL){
    stage("Checkout branch")
    {
        ...
        GIT_TAG_NAME = "omar-download" + "-" + VERSION
    ...
    DOCKER_IMAGE_PATH = "${DOCKER_REGISTRY_PRIVATE_UPLOAD_URL}/omar-download"
                ...
                nodejs(nodeJSInstallationName: "${NODEJS_VERSION}") {
                    ...
                    withSonarQubeEnv('sonarqube'){
                        sh """
                            ...
                            -Dsonar.projectKey=omar-download \
    ...
    stage('Docker build') {
      container('docker') {
        withDockerRegistry(credentialsId: 'dockerCredentials', url: "https://${DOCKER_REGISTRY_DOWNLOAD_URL}") {
          sh """
            docker build --network=host -t "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-download:"${VERSION}" ./docker
          """
        }
      }
    }
    ...
    stage('Docker push'){
      container('docker') {
        ...
        sh """
            docker push "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-download:"${VERSION}"
        """
        }
      }
    }
