  ...
  node(POD_LABEL){
    ...
    stage('Docker build') {
      container('docker') {
        withDockerRegistry(credentialsId: 'dockerCredentials', url: "https://${DOCKER_REGISTRY_DOWNLOAD_URL}") {
          sh """
            docker build --network=host -t "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-download-app:"${VERSION}" ./docker
          """
        }
      }
    }
    ...
    stage('Docker push'){
      container('docker') {
        ...
        sh """
            docker push "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-download-app:"${VERSION}"
        """
        }
      }
    }
